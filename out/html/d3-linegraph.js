...
d3.linegraph = function(noTicks, noDots, parties, partyColors, partyNames, dataMax, dataMin, additionalMonths) {
    /* params */
    if (!parties) {
        parties = ['spd', 'sapd', 'kpd', 'ddp', 'z', 'dvp', 'dnvp', 'nsdap', 'other', 'sbp'];
    }
    if (!partyColors) {
        partyColors = {'spd': '#E3000F', 'sapd': '#FF7F50', 'kpd': '#8B0000', 'ddp': '#DCCA4A', 'z': '#000', 'dvp': '#D5AC27', 'dnvp': '#3f7bc1', 'nsdap': '#954B00', 'other': '#a0a0a0', 'sbp': '#2C6023'};
    }
    if (!partyNames) {
        partyNames = {'spd': 'SPD', 'sapd': 'SAPD', 'kpd': 'KPD', 'ddp': 'DDP', 'z': 'Z + BVP', 'dvp': 'DVP', 'dnvp': 'DNVP', 'nsdap': 'NSDAP', 'other': 'Others', 'sbp': 'SBP'};
    }
    if (!additionalMonths) {
        additionalMonths = 10;
    }
